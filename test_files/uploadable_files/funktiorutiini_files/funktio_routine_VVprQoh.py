...
def generate_params(question_class, lang):
    ...
    if question_class == FUNC_DEFINE:
        ...
        for has_default, param in zip(have_defaults, params):
            ...
            if has_default:
                ppairs.append([pname, repr(param[1])])
            else:
        ...
        formatdict = {
            ...
            "params": ", ".join(str(a) for a in pnames),
            "defaults": ", ".join(
                "{} ({default} {{{{{{#!python3 {}}}}}}})".format(*p, default=defaults_names[lang]) for p in defaults)
        }
        return raw, formatdict
...
def select_ref(question_class, st_code, keywords, lang):
    ...
    if question_class == FUNC_DEFINE:
        ...
        for default in keywords["defaults"]:
            default = default.replace(" (oletusarvo ","=")
            default = default.replace(" (default value ","=")
            default = default.replace(" ","")
            default = default.replace(")","")
            default = default.replace(":", ": ")
            default = default.replace("\'","")
            default_split = default.split("=")
            defaults_values_list.append(default_split[1])
        ...
        #reference function
        values_res = "".join((list(map(str, values_list))))
