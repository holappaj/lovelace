...
programs. It contains several different test functions for testing individual
...
dctm.set_msg("PrintConstructedCode", "en", dict(content="Your answer inserted into a complete program:\n{code}"))
...
dctm.set_msg("PrintConstructedCode", "fi", dict(content="Vastauksesi sijoitettuna kokonaiseen ohjelmaan:\n{code}"))
...
def parse_command():
    ...
    # very ugly backwards compatibility hack
...
def test_code_snippet(st_code, constructor, ref_func, 
    ...
    * *message_validator* - a function that validates the student program's raw 
    ...
    if isinstance(presenter, dict):
        ...
        code_presenter = presenter.get("code", default_construct_presenter)
    else:        
        input_presenter = presenter
        ref_presenter = presenter
        parsed_presenter = presenter
    ...
    if full_code != st_code:
        output(msgs.get_msg("PrintConstructedCode", lang), INFO, code=code_presenter(full_code))
