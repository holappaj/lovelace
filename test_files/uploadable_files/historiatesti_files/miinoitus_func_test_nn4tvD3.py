...
def random_validator(ref, res, parsed):
    ...
    if len(res) == len(temp) and len(res[0]) == len(temp[0]):
        assert count_diff(res, temp), "fail_randomness"
...
if __name__ == "__main__":
    ...
    if st_module:
        ...
        core.test_function(
            st_module, st_function, gen_vector, ref_random_func, lang,
            custom_msgs=rnd_msgs,
            validator=random_validator,
            presenter=rnd_presenters,
            result_object_extractor=field_extractor,
            argument_cloner=copy_room
        )
        if prev:
            core.test_function(
                st_module, st_draw_function, [[]], ref_draw_func, lang,
                custom_msgs=draw_msgs,
                validator=draw_validator,
                presenter=draw_presenters,
                result_object_extractor=draw_extractor,
                new_test=inject_field
            )                   
