...
def default_vars_presenter(module) -> str:
    ...
    for name in sorted(module.__dict__.keys()):
        if not name.startswith("_"):
            var_vals += "{} = {}\n".format(name, getattr(module, name))
...
def parse_command():
    ...
    parser.add_argument("-l", "--locale", "--lang",
        dest="lang",
        default="en",
        help="locale to use for running the test"
    )
    parser.add_argument("-g", "--grind", 
        action="store_const",
        dest="grind",
        const=True,
        default=False,
    ...
    parser.add_argument("-r", "--request-params", 
        action="store_const",
        dest="request",
        const=True,
        default=False,
    ...
    if args.request:
        return None, args.lang
    
    elif args.grind:
        return args.files[0], args.lang
...
def read_grind_json(fname):
    """
    Reads an answer json document for grind type exercises.
    """
    
    with open(fname) as source:
        params = json.load(source)
        
        return params["student_answer"], params["template"], params["variables"], params["values"]
    
def output_grind_json(template, var_names, values, question):
    """
    Shorthand for writing the task instance json document for grind type
    exercises.
    """
    
    json_output.grind_params(template, var_names, values, question)
