...
if __name__ == "__main__":
    ...
    if args.check: 
        ...
        if data["question_class"] == CALL_MODULE_FUNCTION:
            if data["params"]["raw"]["args"] or data["params"]["raw"]["args"] != []:
                formatdict["template_1"] = str(templates[args.lang][4][0]).format(**formatdict)
                formatdict["template_2"] = str(templates[args.lang][4][1]).format(**formatdict)
            else:
                formatdict["template_1"] = str(no_arg_variants[args.lang][4][0]).format(**formatdict)
                formatdict["template_2"] = str(no_arg_variants[args.lang][4][1]).format(**formatdict)
        ...
        raw, formatdict, meta = generate_params(qc, args.lang)
        if qc == CALL_MODULE_FUNCTION:
            if raw["args"] or raw["args"] != []:
                formatdict["template_1"] = str(templates[args.lang][4][0]).format(**formatdict)
                formatdict["template_2"] = str(templates[args.lang][4][1]).format(**formatdict)
            else:
                formatdict["template_1"] = str(no_arg_variants[args.lang][4][0]).format(**formatdict)
