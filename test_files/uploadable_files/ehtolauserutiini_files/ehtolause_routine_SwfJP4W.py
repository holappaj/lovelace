...
COMPOUND = 6
PLAIN_ELSE = 7
FUNC_RETURN_VALUE_COMPARISON = 8
...
boolean_names = {
    True: {"fi": "tosi", "en": "true"},
    False: {"fi": "epätosi", "en": "false"}
}
...
compound_templates = {
    "fi": [
        [
            "onko {{{{{{{var}}}}}}}-muuttujan arvo {operation} kuin {{{{{{#!python3 {val!r}}}}}}}",
        ],
        [
            "onko {{{{{{{var}}}}}}}-muuttujan arvo {boolean_name} (ei vertailuoperaattoreita!)",
        ],
        [
            "onko {{{{{{{var}}}}}}}-muuttujan pituus {operation} kuin {{{{{{#!python3 {val!r}}}}}}}",
        ],
        [
            "onko {{{{{{{var}.{mname}}}}}}}-kutsun paluuarvo {template_args} {boolean_name} (ei vertailuoperaattoreita)",
        ],
        [
            "onko {{{{{{{var}.{mname}}}}}}}-kutsun paluuarvo {template_args} sama kuin {{{{{{#!python3 {val!r}}}}}}}",
        ],
        [
        ...
        [],
        [
            "onko {{{{{{{func}}}}}}}-kutsun paluuarvo argumentilla {{{{{{{var}}}}}}} {operation} kuin {{{{{{{val}}}}}}}"
        ],
    ],
    "en": [
        [
            "is the value of {{{{{{{var}}}}}}} {operation} {{{{{{#!python3 {val!r}}}}}}}",
        ],
        [
            "is the value of {{{{{{{var}}}}}}} {boolean_name} (no comparison operators)",
        ],
        [
            "it the length of {{{{{{{var}}}}}}} {operation} {{{{{{#!python3 {val!r}}}}}}}"
        ],
        [
            "is the return value of {{{{{{{var}.{mname}}}}}}} call {boolean_name} {template_args} (no comparison operators).",
        ],
        [
            "is the return value of {{{{{{{var}.{mname}}}}}}} call equal to {{{{{{#!python3 {val!r}}}}}}} {template_args}.",
        ],
        [
        ...
        [],
        [
            "is the return value of {{{{{{{func}}}}}}} when called with {{{{{{{var}}}}}}} {operation} {{{{{{{val}}}}}}}"
        ],
    ]
}
...
def gen_var(qc="", val="", operation="", boolean="", mname="", args="", var_2="", func=None):
    '''
    ...
    elif qc == FUNC_RETURN_VALUE_COMPARISON:
        return gen_var_val(val, op=operation, func=func)
...
def gen_var_val(val, op="", mname="", args="", code="", func=None):
    '''
    ...
    if func:        
        if func == AbsFunction:
            negatives = gen_var_val(val * -1, op=op)
            positives = gen_var_val(val, op=op)
            return negatives + positives
        if func == IntFunction:
            return [(val + (round(random.random(), 3)), 1), (val - (round(random.random(), 3)), 0)]
        if func == RoundFunction:
            return [
                (val + (round(random.random(), 3)) * 0.5, 1),
                (val - (round(random.random(), 3)) * 0.5, 1),
                (val - 1 + (round(random.random(), 3)) * 0.5, 0),
                (val + 1 - (round(random.random(), 3)) * 0.5, 0)
            ]
...
def generate_params(question_class, lang):
    ...
    if question_class == VARIABLE_VALUE:
        ...
        boolean_name = boolean_names[boolean][lang]
    ...
    if question_class == COMPOUND:
        ...
        for i in range(1, 3):
            ...
            try:
                qcs.remove(COMPOUND)
                qcs.remove(PLAIN_ELSE)
            except:
                pass
            qc = random.choice(qcs)
            qc_tmpl = compound_templates[lang][qc][0]
            raw, kw = generate_params(qc, lang)
            temp_kw["kw_" + str(i)] = kw
            raw["qc"] = qc
            full_kw["template_" + str(i)] = qc_tmpl
            full_raw["raw_" + str(i)] = raw

        if full_raw["raw_1"]["var"] == full_raw["raw_2"]["var"]:
            full_raw["raw_1"]["var"] += "_1"
            full_raw["raw_2"]["var"] += "_2"
            temp_kw["kw_1"]["var"] += "_1"
            temp_kw["kw_2"]["var"] += "_2"
        try:
            if full_raw["raw_1"]["var_2"] == full_raw["raw_2"]["var_2"]:
                full_raw["raw_1"]["var_2"] += "_1"
                full_raw["raw_2"]["var_2"] += "_2"
                temp_kw["kw_1"]["var_2"] += "_1"
                temp_kw["kw_2"]["var_2"] += "_2"
        except KeyError:
            pass
        
        for i in range(1, 3):
            question = full_kw["template_" + str(i)].format(**temp_kw["kw_" + str(i)])
    ...
    if question_class == FUNC_RETURN_VALUE_COMPARISON:
        func = random.choice((AbsFunction, IntFunction, RoundFunction))
        if func == AbsFunction:
            operation = random.choice(COMP_OPS)
            var = random.choice(INT_VAR_NAMES[lang])
            val = random.randint(0, 100)
        elif func == IntFunction:
            operation = COMP_OPS[2]
            var = random.choice(FLOAT_VAR_NAMES[lang])
            val = random.randint(0, 100)
        elif func == RoundFunction:
            operation = COMP_OPS[2]
            var = random.choice(FLOAT_VAR_NAMES[lang])            
            val = random.randint(0, 100)
        raw = {
...
def select_ref(question_class, st_code, keywords, lang, keywords_2=""):
    ...
    if question_class == COMPARISON:
        ...
        test_vars_1 = gen_var(
            keywords_1["qc"],
            val=keywords_1.get("val", ""),
            operation=keywords_1.get("operation", [""])[0],
            args=keywords_1.get("args", ""),
            mname=keywords_1.get("mname", ""),
            var_2=keywords_1.get("var_2", ""),
            boolean=keywords_1.get("boolean", ""),
            func=keywords_1.get("func", "")
        )
        test_vars_2 = gen_var(
            keywords_2["qc"],
        ...
        var_names = []
        var_names.append(keywords_1["var"])
        if "var_2" in keywords_1:
            var_names.append(keywords_1["var_2"])
        var_names.append(keywords_2["var"])
        if "var_2" in keywords_2:
            var_names.append(keywords_2["var_2"])
        test_vars = compiled
    elif question_class == FUNC_RETURN_VALUE_COMPARISON:
        func = eval(keywords["func"])
        test_vars = gen_var(
            question_class,
            val=keywords["val"],
            operation=keywords["operation"][0],
