...
def gen_var_val(val, op="", mname="", args="", code=""):
    ...
    if mname:
        if mname == "count":
            ...
            match = base[:]
            match.extend([val] * args)
            random.shuffle(match)
            match = "".join(match)
            nomatch = base[:]
            nomatch.extend([val] * args * 2)
            nomatch = "".join(nomatch)
            return [(match, 1), (nomatch, 0)]
        if mname == "replace":
            if len(args) == 2:
                match = args[0] * random.randint(3, 6)
                ...
                match = args[0] * args[2]
                nomatch = args[0] * (args[2] + random.randint(1, 4))
            return [(match, 1), (nomatch, 0)]
        if mname == "lower":
            return [(val.upper(), 1), (str(random.randint(0, 9)), 0)]
        if mname == "rstrip":
            return [
        ...
        if mname == "upper":
            return [(val.lower(), 1), (str(random.randint(0, 9)), 0)]
