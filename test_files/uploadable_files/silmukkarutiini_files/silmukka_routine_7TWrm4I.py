...
def select_ref(question_class, st_code, keywords, lang):
    ...
    if question_class == WHILE_VARIABLE:
        false_true_pairs = [
            ...
            ("", "".join(random.sample(string.ascii_letters, random.randint(4, 6)))),
            (0.0, random.random() * 10 + 0.1)
        ]
        pair = random.choice(false_true_pairs)
        target = random.randint(5, 15)
        setattr(ref, res_name[lang], target)
        ...
        def constructor(st_code):
            try:
                ...
                code += 12 * " " + "{} = {}\n".format(
                    keywords["loopvar"],
                    repr(pair[not keywords["boolean"]])
                )
                code += 8 * " " + "if n == 100:\n"
                code += 12 * " " + "break\n"
                code += 4 * " " + "return n\n\n"
                code += "{} = func({})".format(res_name[lang], repr(pair[keywords["boolean"]]))
                return code
            except:
                return st_code
    ...
    if question_class == WHILE_COMPARISON:
        ...
        failsafe = abs(var_ref) * 2 * (var_ref / abs(var_ref))
