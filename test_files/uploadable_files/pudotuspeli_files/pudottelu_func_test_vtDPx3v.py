...
def initial_state_presenter(value):
    ...
    for box in value[0]:    
        svg += "<rect width='{w}' height='{h}' x='{x}' y='{rev_y}' fill='black' />\n".format(**box, rev_y=2 * max_y - box["y"])
...
def fall_presenter(value):
    ...
    for box in value:
        if box["vy"] != 0:
            ...
            fill = "black"
        svg += "<rect width='{w}' height='{h}' x='{x}' y='{rev_y}' fill='{fill}'/>\n".format(**box, fill=fill, rev_y=2 * max_y - box["y"])
    bg = "<rect width='100%' height='100%' fill='white' />\n"
    return "{{{{{{svg|width={w}|height={h:.0f}\n{svg}}}}}}}".format(w=600, h=max_y * 2, svg=bg + svg)
