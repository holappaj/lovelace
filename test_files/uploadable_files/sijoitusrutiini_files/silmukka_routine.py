...
def custom_msgs(question_class, st_code, keywords, constructor_func, var):
    ...
    if question_class == WHILE_VARIABLE:
        custom_msgs.set_msg("PrintStudentResult", "fi", "\nVastauksesi oli:\n{}\n\nTarkistin rakensi ja kutsui funktion:\n{}\n\nTulos oikeassa vastauksessa:\n{}".format(st_code, constructor_func, not keywords["boolean"]))
...
def select_ref(question_class, st_code, keywords, lang):
    ...
    if question_class == WHILE_VARIABLE:
        setattr(ref, res_name[lang], not keywords["loopvar"])

        def constructor(st_code):
            try:
                if (keywords["boolean"] == False) and ("not" not in st_code):
                    raise Exception
                if "while " not in st_code:
                    raise Exception
                code = "def func({}):\n".format(keywords["loopvar"])
                code += "   {}\n".format(st_code)
                code += "       {0} = not {0}\n".format(keywords["loopvar"])
                code += "   return {}\n\n".format(keywords["loopvar"])
                code += "{} = func({})".format(res_name[lang], keywords["boolean"])
                return code
            except:
                return st_code
