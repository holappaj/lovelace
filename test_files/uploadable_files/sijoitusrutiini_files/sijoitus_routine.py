...
def generate_params(question_class, lang):
    if question_class == ASSIGN_VALUE_NUMBER:
        ...
        if vtype == NAME_VALUE:
            ...
            value = random.choice(RANDOM_NAMES[lang])
        elif vtype == PLACE_VALUE:
            var_name = random.choice(PLACE_VAR_NAMES[lang])
            value = random.choice(RANDOM_PLACES[lang])
        raw = {
            "var_name": var_name,
            "value": value
        }
...
def select_ref(question_class, keywords, meta, lang):
    ref = core.SimpleRef()
    if question_class == ASSIGN_VALUE_NUMBER:
        setattr(ref, keywords["var_name"], keywords["value"])
...
if __name__ == "__main__":
    ...
    if args.check: 
        ...
        qc, _, _ = determine_question(data["history"], completed, args.questions)
