...
def gen_string(mname, args, boolean):
    ...
    if boolean == True:
        if mname == "startswith":
            return [(match + body + nomatch, 1), (nomatch + body, 0), (nomatch + match + body + nomatch), 0)]
        elif mname == "endswith":
            return [(nomatch + body + match, 1), (body + nomatch, 0), (nomatch + match + body + nomatch), 0)]
    else:
        if mname == "startswith":
            return [(match + body + nomatch, 0), (nomatch + body, 1), (nomatch + match + body + nomatch), 1)]
        elif mname == "endswith":
            return [(nomatch + body + match, 0), (body + nomatch, 1), (nomatch + match + body + nomatch), 1)]
