...
def gen_var_val(val, op="", mname="", args="", code=""):
    ...
    if op:
        if op == "<":
            return [(val / 2, 1), (val, 0), (val * 2, 0)]
        elif op == "<=":
            return [(val / 2, 1), (val, 1), (val * 2, 0)]
        elif op == ">":
            return [(val * 2, 1), (val, 0), (val / 2, 0)]
        elif op == ">=":
            return [(val * 2, 1), (val, 1), (val / 2, 0)]
        elif op == "==":
            return [(val * 2, 0), (val, 1), (val / 2, 0)]
        elif op == "!=":
            return [(val * 2, 1), (val, 0), (val / 2, 1)]
...
def select_ref(question_class, st_code, keywords, lang, keywords_2=""):
    ...
    if question_class == COMPARISON:
        ...
        test_vars = gen_var(question_class, boolean=keywords["boolean"])
    ...
    def constructor(st_code):
        try: 
            ...
            for i, (var, _) in enumerate(test_vars, start=1): 
                code += "{} = func({})".format("{}_{}".format(res_name[lang], i), var)
            return code
        except:
            return st_code
